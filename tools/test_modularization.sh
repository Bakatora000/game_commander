...
EOF
    ...
    grep -q 'tools/nginx_manager.py init --domain gaming.example.com --manifest /etc/nginx/game-commander-manifest.json --loc-file /etc/nginx/game-commander-locations.conf --hub-file /etc/nginx/game-commander-hub.html --hub-port 5090 --backup-dir /etc/nginx/backups' "$sandbox/calls.log" || return 1
    ...
    grep -q 'tools/nginx_manager.py regenerate --manifest /etc/nginx/game-commander-manifest.json --out /etc/nginx/game-commander-locations.conf --hub-file /etc/nginx/game-commander-hub.html --hub-port 5090' "$sandbox/calls.log" || return 1
